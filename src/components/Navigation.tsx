...
import { Menu, X, Home, User, FolderOpen, Code, Mail, Briefcase } from "lucide-react";
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    key={item.id}
                    onClick={() => scrollToSection(item.id)}
                    className={`flex items-center gap-3 w-full text-left px-0 py-3 text-sm font-medium transition-all duration-300 hover:bg-primary/10 rounded-lg px-4 ${
                      activeSection === item.id 
